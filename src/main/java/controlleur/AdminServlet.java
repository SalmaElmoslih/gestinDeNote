...
public class AdminServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 String action = request.getParameter("action");
		    ...
		    else if(action.equalsIgnoreCase("ajouterMod")){
		    	 request.getRequestDispatcher("ajouterMod.jsp").forward(request, response);	
		    }
		    else if(action.equalsIgnoreCase("afficherModule")) {
		    	
				 Modules mod = (Modules)request.getSession().getAttribute("mod");
				 List<Modules> modd = (List<Modules>) moduleRep.findAll();
		         HttpSession httpSession = request.getSession();
		         httpSession.setAttribute("liste", modd);
		    	 request.getRequestDispatcher("afficherModule.jsp").forward(request, response);
	        } 
		    else if(action.equalsIgnoreCase("modifier_Module")) {
		        int idParameter = Integer.parseInt(request.getParameter("id_M"));
		        Modules moduleToModify = moduleRep.findById(idParameter);
		        request.setAttribute("mod", moduleToModify);
		        request.getRequestDispatcher("modifier_Module.jsp").forward(request, response);
		    }
		    else if(action.equalsIgnoreCase("delete_Module")) {
		        int idParameter = Integer.parseInt(request.getParameter("id_S"));
		        //etudiant etudiantToDelete = etdRep.findById(idParameter);
		        moduleRep.deleteById(idParameter);
		        response.sendRedirect("admin?action=afficherModule");
		    }
...
private void doAjouterMod(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	String nom = request.getParameter("nom");
	    ...
	    request.getRequestDispatcher("detailModule.jsp").forward(request, response);
}
private void doModifietMod(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if (moduleToModify == null) {
        response.getWriter().write("Le module à modifier n'existe pas.");
        return;
    }
    ...
    moduleToModify.setNom(nom); 

    moduleRep.modify(moduleToModify);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (action) {
            ...
            case "ajouterMod": 
                try {
                	doAjouterMod(request, response);
                } catch (Exception e) {
                    e.printStackTrace();
                    response.getWriter().write("Error during ajout module: " + e.getMessage());
                }
                break;
            case "modifier_Module": 
                try {
                	doModifietMod(request, response);
                } catch (Exception e) {
                    e.printStackTrace();
                    response.getWriter().write("Error during modifier module: " + e.getMessage());
                }
                break;
            default:
